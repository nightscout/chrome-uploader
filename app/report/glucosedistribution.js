function generate_report(data, high, low) {
	require(["../bloodsugar"], function(convertBg) {
		...
		if (data.length > 0) {
			...
			$("<td><center>" + Math.round(10*(ss.mean(localBgs)+46.7)/28.7)/10 + "%<sub>DCCT</sub> | " +Math.round(((ss.mean(localBgs)+46.7)/28.7 - 2.15)*10.929) + "<sub>IFCC</sub></center></td>").appendTo(tr);
		} else {
			$("<td>N/A</td>").appendTo(tr);
			$("<td>N/A</td>").appendTo(tr);
			$("<td>N/A</td>").appendTo(tr);
